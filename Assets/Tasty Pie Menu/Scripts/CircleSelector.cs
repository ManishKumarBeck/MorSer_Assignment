...
//                    vector.y = vector.y*((flip) ? -1 : 1);
                    if (tiltTowardsMouse)
                    {
                        float x = vector.x / screenBounds.x;
                        float y = vector.y / screenBounds.y;

                        Vector3 targetEuler = (Vector3)(new Vector2(y, -x) * -tiltAmount) + Vector3.forward * zRotation;

                        // Clamp X and Y axes
                        targetEuler.x = Mathf.Clamp(targetEuler.x, -30f, 30f);
                        targetEuler.y = Mathf.Clamp(targetEuler.y, -15f, 25f);

                        Quaternion targetRotation = Quaternion.Euler(targetEuler);

                        transform.localRotation = Quaternion.Slerp(transform.localRotation, targetRotation, LerpAmount);
                    }
                        ...
                        !selectOnlyOnHover)
                    {
                        ...
                        instancedButtons[SelectedSegment].SetColor(
                         ...
                         //#Debug.Log($"Selected Segment: {instancedButtons[SelectedSegment].id}");
                         toolName.text = instancedButtons[SelectedSegment].id.ToString();
